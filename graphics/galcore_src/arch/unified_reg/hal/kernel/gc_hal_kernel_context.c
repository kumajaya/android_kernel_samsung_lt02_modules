    ...
    )
{
...
#endif
        gctUINT32  BufferPhysical = gcvINVALID_ADDRESS;
        gcmkHEADER_ARG("Os=0x%08X Hardware=0x%08X", Os, Hardware);
#ifdef CNTBUF_OLDLOGIC
        buffer->oldlogic = gcvTRUE;
        ...
        gcmkONERROR(gckOS_GetPhysicalAddressByHandle(Os,buffer->logical,buffer->physical,&BufferPhysical));

        context->node = gcvNULL;
#else
        {
            buffer->oldlogic = gcvFALSE;
            gcmkONERROR(gckVIDMEM_ConstructVirtual(Hardware->kernel, gcvFALSE, gcvFALSE, context->totalSize, &(context->node)));
            buffer->physical = context->node->Virtual.physical;

            gcmkONERROR(gckVIDMEM_Lock(Hardware->kernel,
                                        context->node,
                                        gcvFALSE,
                                        &BufferPhysical));
            buffer->logical =(gctUINT32_PTR)context->node->Virtual.logical;
        }

#endif
        gcmkONERROR(gckOS_SetGCPhysByMdl(Os, buffer->physical, BufferPhysical));
    ...
    )
{
    ...
    {
        gctUINT32    ProcessID;
        gctBOOL      Asynchroneous = 0;
        if(buffer->logical)
        {
            gckOS_GetProcessID(&ProcessID);
            gcmkONERROR(
                gckVIDMEM_Unlock(Context->hardware->kernel,
                                 Context->node,
                                 (gctUINT)ProcessID,
                                 gcvSURF_BITMAP,
                                 &Asynchroneous,
    ...
    )
{
    ...
    if (Context != gcvNULL)
    {
        ...
        for (bufferHead = Context->buffer; Context->buffer != gcvNULL;)
        {
            ...
            if (buffer->logical != gcvNULL)
            {
                gcmkONERROR(_gckCntxBuffer_Free(Context,buffer));
            }
    ...
    )
{
    ...
    for (i = 0; i < gcdCONTEXT_BUFFER_COUNT; i += 1)
    {
        ...
        gcmkONERROR(_gckCntxBuffer_Allocate(Os, Hardware, context, buffer));
...
#if gcmIS_DEBUG(gcdDEBUG_CODE) && !defined(VIVANTE_NO_3D)
    ...
    if (buffer->deltaCount != 0)
    {
        ...
        for (i = 0; i < buffer->deltaCount; i += 1)
        {
            ...
            for (j = 0; j < kDelta->recordCount; j += 1)
            {
                ...
                if (index == 0)
                {
#if (gcdDEBUG || MRVL_ALWAYS_PRINT_ERROR)
                    if ((address != 0x0594)
                     && (address != 0x0E00)
                     && (address != 0x0E03)
                        )
