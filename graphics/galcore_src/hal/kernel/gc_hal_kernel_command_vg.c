    ...
    if (node != gcvNULL)
    {
        ...
        if (address != ~0)
        {
            gcmkCHECK_STATUS(gckVIDMEM_Unlock(
                Command->kernel->kernel, node, gcvSURF_TYPE_UNKNOWN, gcvNULL, gcvFALSE
                ));
        }
    ...
    )
{
    ...
    do
    {
        ...
        gcmkERR_BREAK(gckVIDMEM_Unlock(Kernel->kernel, Node, gcvSURF_TYPE_UNKNOWN, gcvNULL, gcvFALSE));
