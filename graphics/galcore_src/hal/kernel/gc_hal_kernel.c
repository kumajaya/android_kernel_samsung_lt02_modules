...
** _UsePmem
**
** Calculate whether use PMEM memory or reserved memory to reduce memory fragment
**
...
**
** Reserved memory to avoid too many fragment, else use Pmem.
**
*/
static inline gctBOOL _UsePmem(IN gctSIZE_T Bytes)
{
    gctSIZE_T wasteBytes = (PAGE_SIZE - (Bytes & (PAGE_SIZE - 1))) % PAGE_SIZE;

    /* For those memory size less than 512k, let them go to reserve memory to allocate. */
    if(Bytes <= (512 << 10))
    {
        return gcvFALSE;
    }
    /* WasteBytes /Bytes < 0.1. */
    else if((wasteBytes * 10) < Bytes)
    {
        return gcvTRUE;
    }
    else
    ...
    )
{
    ...
    do
    {
        ...
        {
            /* Try to allocate memory from pmem firstly. */
            gckVIDMEM_AllocateMemory(gcvTRUE, gcvTRUE);

            /* Try to allocate memory from reserve if pmem failed. */
        ...
        else
        {
            ...
            else
            {
                ...
                /* Try to allocate memory from pmem if reserved failed. */
                gckVIDMEM_AllocateMemory(gcvTRUE, gcvTRUE);
            }
    ...
    else
    {
        gcmkTRACE(gcvLEVEL_ERROR,
            "%s(%d), _AllocateMemory failed, Type %d, Pool %d, Byte %d, UsePmem %d",
            __FUNCTION__, __LINE__,
            Type, *Pool, Bytes, _UsePmem(Bytes));
    }
