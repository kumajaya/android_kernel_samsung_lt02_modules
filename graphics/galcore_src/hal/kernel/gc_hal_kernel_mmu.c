...
{
#if gcdENABLE_4K_MMU_CONGIG
    gckOS_Log(_GFX_LOG_NOTIFY_, "GC ENABLE_4K_MMU_CONGIG");
    if(xres*yres >= 540*960)
    {
        return gcvTRUE;
    }
    else
...
{
#if gcdENABLE_SET_LIMITED_OUTSTANDING
    ...
    )
{
    ...
    if(_IsBigPanel4kMmuConfig(registered_fb[0]->var.xres,registered_fb[0]->var.yres))
    {
    ...
    )
{
    ...
    /* Map to Master TLB. */
    for (i = dynamicMapStart; i <= dynamicMapEnd; i++)
    {
    ...
    )
{
    ...
    {
        hardware->outstandingLimitation = _SetBigPanelLimitedOutStanding(registered_fb[0]->var.xres, registered_fb[0]->var.yres);
    }
    else
    {
