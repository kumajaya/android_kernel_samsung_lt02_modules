...
/** Version */
#define VERSION "M2614016"

/** Driver version */
char driver_version[] = "sd8787-%s-" VERSION "-(" "FP" FPNUM ")"
#ifdef DEBUG_LEVEL2
    "-dbg"
#endif
    " ";

char fw_version[32] = "0.0.0.p0";
...
bt_send_reset_command(bt_private * priv)
{
    ...
    if (!os_wait_interruptible_timeout
        (priv->adapter->cmd_wait_q, priv->adapter->cmd_complete,
         WAIT_UNTIL_CMD_RESP)) {
        ret = BT_STATUS_FAILURE;
        PRINTM(MSG, "BT: Reset version: timeout: %d, %d,%d\n",
...
/**
 *  @brief This function set initial power. 2012/10/25. by deniskim
 *
 *  @param priv    A pointer to bt_private structure
 ...
 */
int
bt_set_initial_power(bt_private * priv, u8 * mac)
{
	struct sk_buff *skb = NULL;
	int ret = BT_STATUS_SUCCESS;
	BT_HCI_CAL_CMD *pCmd;
	int i = 0;
	u8 write_cal[32]={0x00,0x00,0x01,0x1C,
						0x37,0x00,0x1C,0x00,
						0x00,0x00,0x00,0x00,
						0x00,0x00,0x08,0x04,
						0x00,0x00,0xBA,0xCE,
	...
	skb = bt_skb_alloc(sizeof(BT_HCI_CAL_CMD), GFP_ATOMIC);
	if (skb == NULL) {
		PRINTM(WARN, "No free skb\n");
		ret = BT_STATUS_FAILURE;
		goto exit;
	...
	pCmd = (BT_HCI_CAL_CMD *) skb->tail;
	pCmd->ocf_ogf = (OGF << 10) | BT_CMD_WRITE_CAL_DATA;
	pCmd->length = 32;

	for (i = 0; i < 6; i++)
		write_cal[24+i] = mac[5-i];
	for (i = 0; i < 32; i++)
		pCmd->data[i] = write_cal[i];

	bt_cb(skb)->pkt_type = HCI_COMMAND_PKT;
	skb_put(skb, sizeof(BT_HCI_CAL_CMD));
	skb->dev = (void *) priv->bt_dev.hcidev;
	skb_queue_head(&priv->adapter->tx_queue, skb);
	priv->bt_dev.sendcmdflag = TRUE;
	priv->bt_dev.send_cmd_ocf = BT_CMD_WRITE_CAL_DATA;
	priv->adapter->cmd_complete = FALSE;
	PRINTM(CMD, "BT: Set mac address %02x:%02x:%02x:%02x:%02x:%02x (0x%x)\n",
				mac[0], mac[1], mac[2], mac[3], mac[4], mac[5], pCmd->ocf_ogf);
	wake_up_interruptible(&priv->MainThread.waitQ);
	if (!os_wait_interruptible_timeout(priv->adapter->cmd_wait_q, priv->adapter->cmd_complete, WAIT_UNTIL_CMD_RESP)) {
			ret = BT_STATUS_FAILURE;
			PRINTM(ERROR, "BT: Set mac address: timeout: %d, %d,%d\n",
			priv->adapter->cmd_complete, priv->adapter->ps_state,
			priv->adapter->WakeupTries);
	}
...
sd_register_conf_dpc(bt_private * priv)
{
	...
	if (mac_addr)
	{
		u8 bt_mac[ETH_ALEN]={0x12,0x22,0x33,0x44,0x55,0x66};
		u8 bd_addr[MAX_MAC_ADDR_LEN];
		memcpy(bd_addr, mac_addr,MAX_MAC_ADDR_LEN -1 );
		bd_addr[MAX_MAC_ADDR_LEN -1]='\0';
		//PRINTM(ERROR, "### eric #### bd_addr = ");
		//PRINTM(ERROR, "### eric #### read bd_addr = %s\n", bd_addr);
		bt_mac_to_u8(bt_mac, bd_addr);
		//PRINTM(ERROR, " new BT Address %02x:%02x:%02x:%02x:%02x:%02x\n",
		//		bt_mac[0], bt_mac[1], bt_mac[2], bt_mac[3],
		//		bt_mac[4], bt_mac[5]);
		bt_set_initial_power(priv, bt_mac);
		PRINTM(ERROR, "%s: SET INITIAL POWER", __FUNCTION__);
	}
    /* Get FW version */
...
MODULE_PARM_DESC(mac_addr, "BT write device address");
